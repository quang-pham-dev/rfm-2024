...
export function useFloatingActionEvents() {
  ...
  React.useEffect(() => {
    ...
    if (
      scrollState === Scroll.onMomentumScrollEnd ||
      scrollState === Scroll.onScrollEndDrag
    ) {
      timeout = setTimeout(() => {
